...
| `PAPERCLIP_REQUIRE_GOVERNED_AGENT_CREATION` | `false` | Deny `agents:create` to agent principals (overriding both explicit grants and legacy CEO authority), and require `hire_agent` approvals to reference a pre-created `pending_approval` agent so approving one activates rather than mints. Board principals keep every native create, hire, import, and catalog path. Intended for deployments where an external control plane owns agent limits and provisioning. |
